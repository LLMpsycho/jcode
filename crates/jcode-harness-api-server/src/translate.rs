...
impl BridgeState {
    ...
    fn resolve_session_metadata(session_id: &str) -> Option<PersistedSessionMetadata> {
        ...
        let read_windows = || -> std::io::Result<(Vec<u8>, Vec<u8>)> {
            let mut file = std::fs::File::open(path)?;
            let len = file.metadata()?.len();
            // The minimum is bounded by WINDOW and therefore fits usize.
            let head_len = len.min(WINDOW as u64) as usize;
        ...
        let (head, tail) = match read_windows() {
            Ok(windows) => windows,
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => return None,
            Err(error) => {
                eprintln!("harness API bridge: session metadata read failed: {error}");
                return None;
            }
        };
    ...
    fn transcript_bytes(session_id: &str) -> Option<u64> {
        ...
        match std::fs::metadata(path) {
            ...
            Err(error) => {
                eprintln!("harness API bridge: transcript size lookup failed: {error}");
                None
            }
        }
    }
    ...
    fn session_modified_ms(session_id: &str) -> Option<u64> {
        let modified = match Self::session_record_path(session_id)?
            .metadata()
            .and_then(|metadata| metadata.modified())
        {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => return None,
            Err(error) => {
                eprintln!("harness API bridge: session modification time lookup failed: {error}");
                return None;
            }
        };
        match modified.duration_since(UNIX_EPOCH) {
            Ok(duration) => Some(duration.as_millis().min(u64::MAX as u128) as u64),
            Err(_) => None,
        }
    }
    ...
    fn recent_session_index_entries() -> Vec<RecentSessionIndexEntry> {
        ...
        if connection
            .prepare("SELECT saved FROM recent_sessions LIMIT 0")
            .is_err()
            && let Err(error) = connection.execute(
                "ALTER TABLE recent_sessions ADD COLUMN saved INTEGER NOT NULL DEFAULT 0",
                [],
            )
        {
            eprintln!("harness API bridge: recent session index migration failed: {error}");
            return Vec::new();
        }
        ...
        statement
            ...
            .unwrap_or_else(|error| {
                eprintln!("harness API bridge: recent session index read failed: {error}");
                Vec::new()
            })
    }
    ...
    fn write_bootstrap_recent_session_index(ids: &[(SystemTime, String)]) {
        ...
        for (modified, session_id) in ids.iter().take(500) {
            let metadata = Self::resolve_session_metadata(session_id)
                .unwrap_or_else(PersistedSessionMetadata::default);
            ...
            if let Err(error) = transaction.execute(
                ...
                eprintln!("harness API bridge: recent session index bootstrap failed: {error}");
                return; // Dropping the transaction rolls back the partial bootstrap.
            }
        }
        if let Err(error) = transaction.commit() {
            eprintln!("harness API bridge: recent session index commit failed: {error}");
        }
    }
            ...
            handles
                .flat_map(|handle| {
                    handle.join().unwrap_or_else(|_| {
                        eprintln!("harness API bridge: session metadata worker panicked");
                        Vec::new()
                    })
                })
                .collect::<Vec<_>>()
