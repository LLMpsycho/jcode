...
fn advisor_wire_controls_match_the_production_protocol() {
    for request in [
        ...
        daemon::AdvisorRequest::Acknowledge { note_id: "adv-1".into() },
        daemon::AdvisorRequest::Dismiss { note_id: "adv-2".into() },
        daemon::AdvisorRequest::ModelOptions { selection: None },
        daemon::AdvisorRequest::ModelOptions { selection: Some(selection()) },
        ...
        let frame = ClientFrame::new(7, ApiRequest::Advisor {
            session_id: "s1".into(), request: public,
        });
        ...
        assert!(state.legacy_event_to_api(&json!({"type": "ack", "id": id})).is_empty());
        ...
        assert!(matches!(&result[0].event, ApiEvent::AdvisorResult { result, .. } if result.error.is_none()));
    }
}
...
fn advisor_runtime_tags_match_every_serializable_production_identity() {
    for key in [
        RuntimeKey::JcodeSubscription, RuntimeKey::ClaudeOAuth, RuntimeKey::AnthropicApiKey,
        RuntimeKey::OpenAIOAuth, RuntimeKey::OpenAIApiKey, RuntimeKey::OpenRouter,
        RuntimeKey::OpenAiCompatible { profile_id: None },
        RuntimeKey::OpenAiCompatible { profile_id: Some("work-profile".into()) },
        RuntimeKey::Copilot, RuntimeKey::Gemini, RuntimeKey::Cursor, RuntimeKey::Bedrock,
        RuntimeKey::Antigravity, RuntimeKey::CodeAssistOAuth, RuntimeKey::RemoteCatalog,
        RuntimeKey::Current,
    ] {
        let key_json = serde_json::to_value(&key).unwrap();
        assert!(known_runtime_kind(key_json["kind"].as_str().unwrap()), "unrecognized {key:?}");
        let mut route = selection();
        route.runtime_key = key;
        let action = serde_json::to_value(daemon::AdvisorRequest::SelectModel {
            selection: route, reasoning_effort: None,
        }).unwrap();
...
fn advisor_model_options_preserve_canonical_routes_and_legacy_defaults() {
    let mut state = attached();
    let legacy = forwarded(state.api_request_to_legacy(&control(3, json!({"action": "model_options"}))));
    let event = daemon::ServerEvent::AdvisorResult {
        ...
        result: daemon::AdvisorControlResult {
            ...
            model_settings: Some(daemon::AdvisorModelSettings {
                enabled: true, selection: Some(selection()), reasoning_effort: Some("high".into()),
                follows_primary: false,
            }),
            model_options: Some(daemon::AdvisorModelOptions {
                selection: Some(selection()), reasoning_effort: Some("high".into()),
                available_routes: vec![ModelRoute {
                    model: "gpt-5".into(), provider: "OpenAI".into(), api_method: "OpenAI OAuth".into(),
                    available: true, detail: "subscription".into(), cheapness: None,
                }],
    ...
    assert_eq!(serde_json::to_value(&options.available_selections[0]).unwrap(), serde_json::to_value(selection()).unwrap());
    ...
    assert!(serde_json::to_value(old).unwrap().get("available_selections").is_none());
}
...
fn advisor_control_replies_do_not_complete_or_consume_a_primary_turn() {
    ...
    let wrong = json!({"type": "advisor_result", "id": turn["id"], "result": {"message": "wrong id"}});
...
fn advisor_controls_reject_wrong_sessions_and_malformed_requests_locally() {
    ...
    for (mut state, request) in [
        ...
        (attached(), control(4, json!({"action": "select_model", "selection": bad_runtime}))),
    ] {
        let replies = state.api_request_to_legacy(&request);
        assert!(matches!(&replies[0], Outbound::Reply(ServerFrame {
            reply_to: Some(4), event: ApiEvent::Error { .. }, ..
        })));
...
fn advisor_replies_keep_the_originating_session_and_report_durability_errors() {
    ...
    assert!(matches!(&replies[0].event, ApiEvent::AdvisorResult { session_id, result }
...
fn advisor_malformed_results_fail_the_waiter_without_later_duplicate_replies() {
    let mut state = attached();
    let request = forwarded(state.api_request_to_legacy(&control(4, json!({"action": "enable"}))));
    let malformed = json!({"type": "advisor_result", "id": request["id"], "result": {"message": 12}});
    let replies = state.legacy_event_to_api(&malformed);
    assert_eq!(replies[0].reply_to, Some(4));
    assert!(matches!(replies[0].event, ApiEvent::Error { code: ErrorCode::Internal, .. }));
