...
pub(super) fn handle_advisor(
    ...
    handle_with_manager(id, request, session, agent, events, crate::advisor::advisor_manager());
}
...
fn handle_with_manager(
    ...
    if matches!(request, AdvisorRequest::SelectModel { .. } | AdvisorRequest::UsePrimary | AdvisorRequest::ModelOptions { .. }) {
        let selection_id = if matches!(request, AdvisorRequest::SelectModel { .. } | AdvisorRequest::UsePrimary) {
            manager.begin_model_selection(session)
        } else { 0 };
        if let Ok(agent) = agent.try_lock() {
            let provider = agent.provider_handle();
            drop(agent);
            let result = model_request(&manager, session, provider.as_ref(), &config, request, selection_id);
...
fn model_request(
    ...
    let outcome = match request {
        ...
        AdvisorRequest::SelectModel { selection, reasoning_effort } => manager
            .select_model(session, provider, config, selection, reasoning_effort, selection_id)
            .map(|settings| {
                result.message = format!("Advisor enabled for this session: {}.", settings_label(&settings));
            }),
        AdvisorRequest::UsePrimary => manager.use_primary_model(session, provider, config, selection_id).map(|_| {
            result.message = "Advisor enabled and following the primary model and effort for this session.".into();
        }),
...
fn settings_label(settings: &crate::protocol::AdvisorModelSettings) -> String {
    let model = settings.selection.as_ref().map(|route| {
        format!("{} via {} ({})", route.model, route.provider_label, route.api_method)
...
fn control_request(
    ...
    let message = match request {
        AdvisorRequest::Status => {
            ...
            match manager.snapshot(session) {
                Some(snapshot) => format!(
                    ...
                    if enabled { "on" } else { "off" }, snapshot.status,
                    snapshot.unresolved_blocking_notes, manager.notes(session).len(),
                    snapshot.last_error.as_deref().unwrap_or("no error")
                ),
                None => format!("Advisor: {} (idle); no retained notes", if enabled { "on" } else { "off" }),
            }
        }
        AdvisorRequest::Inspect => {
            ...
            if notes.is_empty() {
                ...
                notes.into_iter().map(|note| format!(
                    "{} [{:?}/{:?}] {}\nRecommended: {}{}", note.id, note.severity,
                    note.disposition, note.summary, note.recommended_action,
                    if note.evidence.is_empty() { String::new() } else { format!("\nEvidence: {}", note.evidence.join(" | ")) }
                )).collect::<Vec<_>>().join("\n\n")
            }
        }
        AdvisorRequest::Dismiss { note_id } => match manager.resolve_note(session, &note_id, AdvisorNoteDisposition::Dismissed) {
            Ok(true) => format!("Dismissed advisor note {note_id}."),
            Ok(false) => format!("Advisor note {note_id} was not found."),
            Err(error) => error.to_string(),
        },
        AdvisorRequest::Acknowledge { note_id } => match manager.resolve_note(session, &note_id, AdvisorNoteDisposition::Acknowledged) {
    ...
    AdvisorControlResult { message, ..AdvisorControlResult::default() }
}
