//! Bounded evidence extraction for a completed primary turn.
//!
...
fn summarize_diff(calls: &[(&str, &Value, Option<&SourceResult>)]) -> String {
    ...
    for (name, input, _) in calls {
        ...
        let action = input.get("action").and_then(Value::as_str).unwrap_or(name);
...
fn summarize_acceptance(plan: &crate::todo::TodoPlan, goals: &[crate::todo::TodoGoal]) -> String {
    let mut lines = Vec::new();
...
fn collect_named_strings(value: &Value, keys: &[&str], output: &mut Vec<String>, limit: usize) {
    if output.len() >= limit {
        return;
    }
    match value {
...
mod tests {
    ...
    fn extracts_bounded_diff_diagnostics_and_verification_without_raw_replay() {
        ...
        assert!(
            !input
                .diagnostics
                .contains("raw payload raw payload raw payload")
        );
    }
