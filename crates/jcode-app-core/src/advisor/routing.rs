...
fn permitted(route: &ModelRoute, config: &AdvisorConfig) -> bool {
    route.available && config.allowed_runtime_keys.as_ref().is_none_or(|keys| {
        keys.contains(&RouteSelection::from_model_route(route).runtime_key.stable_id())
    })
}
...
fn current_runtime(route: &ModelRoute, provider: &dyn Provider) -> bool {
    ...
    match (runtime, provider.active_resolved_credential()) {
        ...
        (RuntimeKey::AnthropicApiKey | RuntimeKey::OpenAIApiKey, Some(auth)) => !auth.is_subscription(),
        _ => true,
    }
}
...
pub(super) fn apply(provider: &dyn Provider, config: &AdvisorConfig) -> Result<()> {
    ...
    if requested.is_empty() { bail!("empty advisor model route"); }
    let mut routes: Vec<_> = provider.model_routes().into_iter().filter(|route| {
    ...
    if routes.len() > 1 {
        let current: Vec<_> = routes.iter().filter(|route| current_runtime(route, provider)).cloned().collect();
        if current.len() == 1 { routes = current; }
    }
    if routes.len() != 1 {
        bail!("advisor role has no unique permitted, available route; select a canonical /model route and check allowed_runtime_keys");
    }
    if request.is_some() {
...
mod tests {
    ...
    impl Provider for CatalogProvider {
        ...
        fn model(&self) -> String { "coder".into() }
        ...
        fn fork(&self) -> Arc<dyn Provider> {
            Arc::new(Self { routes: self.routes.clone(), selected: Mutex::new(None) })
        }
    }

    fn provider() -> CatalogProvider {
        CatalogProvider { routes: vec![
            ModelRoute { model: "reviewer".into(), provider: "OpenAI".into(), api_method: "openai-api".into(), available: true, detail: String::new(), cheapness: None },
            ModelRoute { model: "verifier".into(), provider: "OpenAI".into(), api_method: "openai-api".into(), available: true, detail: String::new(), cheapness: None },
    ...
    fn mode_selects_its_role_only_on_the_fork() {
        ...
        let config = AdvisorConfig { reviewer_model: Some("reviewer".into()), verification_model: Some("verifier".into()), ..AdvisorConfig::default() };
        apply(fork.as_ref(), &config).expect("reviewer");
        assert!(primary.selected.lock().expect("selection").is_none());
        apply(&primary, &AdvisorConfig { mode: AdvisorMode::FinalReview, ..config }).expect("verifier");
        assert_eq!(primary.selected.lock().expect("selection").as_ref().expect("route").model, "verifier");
    }

    #[test]
    fn denied_unavailable_or_ambiguous_roles_fail_before_selection() {
        let mut primary = provider();
        let config = AdvisorConfig { model: Some("reviewer".into()), allowed_runtime_keys: Some(vec![]), ..AdvisorConfig::default() };
        assert!(apply(&primary, &config).is_err());
        assert!(primary.selected.lock().expect("selection").is_none());
        primary.routes[0].available = false;
        assert!(apply(&primary, &AdvisorConfig { allowed_runtime_keys: None, ..config }).is_err());
        let config = AdvisorConfig { model: Some("not-in-catalog".into()), ..AdvisorConfig::default() };
        assert!(apply(&primary, &config).is_err());
    }
}
