                ...
                match entry.action {
                    ...
                    PickerAction::SubagentModelChoice { inherit } => {
                        ...
                        if let Err(error) = self.session.save() {
                            ...
                            self.push_display_message(DisplayMessage::error(format!(
                                "Model selected, but session settings could not be saved: {error}"
                            )));
                        }
                    }
                    PickerAction::Model => {
                            ...
                            match self.provider.set_route_selection(&route_selection) {
                                Ok(()) => {
                                    ...
                                    if let Err(error) = self.session.save() {
                                        ...
                                        self.push_display_message(DisplayMessage::error(format!("Model selected, but session settings could not be saved: {error}")));
                                    }
                                    crate::logging::event_info(
...
#[cfg(test)]
#[path = "inline_interactive_model_tests.rs"]
mod tests;
