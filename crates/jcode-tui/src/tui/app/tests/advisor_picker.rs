...
fn advisor_test_result(selection: Option<crate::provider::RouteSelection>) -> crate::protocol::AdvisorControlResult {
    crate::protocol::AdvisorControlResult {
        model_options: Some(crate::protocol::AdvisorModelOptions {
            selection,
            reasoning_effort: Some("medium".into()),
            available_routes: vec![advisor_test_route("openai-oauth", true), advisor_test_route("openai-api", false)],
...
async fn advisor_read_request(app: &mut App, remote: &mut crate::tui::backend::RemoteConnection, reader: &mut tokio::io::BufReader<crate::transport::ReadHalf>) -> (u64, serde_json::Value) {
...
fn advisor_command_opens_picker_and_preserves_controls() {
    ...
    assert_eq!(command("/advisor inherit").unwrap().unwrap(), AdvisorRequest::UsePrimary);
    assert_eq!(command("/advisor status").unwrap().unwrap(), AdvisorRequest::Status);
    assert_eq!(command("/advisor off").unwrap().unwrap(), AdvisorRequest::Disable);
    assert_eq!(command("/advisor ack adv_1").unwrap().unwrap(), AdvisorRequest::Acknowledge { note_id: "adv_1".into() });
...
fn advisor_picker_selects_oauth_route_and_effort_without_switching_main() {
    with_temp_jcode_home(|| {
        ...
        rt.block_on(async {
            ...
            assert_eq!(picker.entries.len(), 2, "inherit plus authenticated OAuth route; no API key needed");
            ...
            let mut route = crate::provider::RouteSelection::from_model_route(&advisor_test_route("openai-oauth", true));
            route.detail.clear();
...
fn advisor_picker_cancel_and_session_change_ignore_late_results() {
    ...
    rt.block_on(async {
        ...
        for switch_session in [false, true] {
            app.queue_advisor_request(crate::protocol::AdvisorRequest::ModelOptions { selection: None });
...
fn advisor_picker_uses_supported_efforts_and_does_not_save_main_defaults() {
    with_temp_jcode_home(|| {
        ...
        rt.block_on(async {
            ...
            let (peer, _writer) = remote.take_dummy_peer().unwrap().into_split();
            let mut reader = tokio::io::BufReader::new(peer);
            let route = crate::provider::RouteSelection::from_model_route(&advisor_test_route("openai-oauth", true));
            app.queue_advisor_request(crate::protocol::AdvisorRequest::ModelOptions { selection: Some(route.clone()) });
            let (id, _) = advisor_read_request(&mut app, &mut remote, &mut reader).await;
            let mut result = advisor_test_result(Some(route));
            result.model_options.as_mut().unwrap().available_efforts = vec!["swarm".into(), "swarm-deep".into()];
            ...
            app.handle_inline_interactive_key(KeyCode::Char('o'), KeyModifiers::CONTROL).unwrap();
            app.handle_inline_interactive_key(KeyCode::Char('n'), KeyModifiers::CONTROL).unwrap();
...
fn advisor_picker_displays_server_rejection_and_ignores_old_session_controls() {
    ...
    rt.block_on(async {
        ...
        let (id, _) = advisor_read_request(&mut app, &mut remote, &mut reader).await;
        app.handle_advisor_result(id, crate::protocol::AdvisorControlResult {
            error: Some("This route is not permitted for the advisor".into()),
            ..Default::default()
        });
        assert!(app.display_messages().last().unwrap().content.contains("not permitted"));
