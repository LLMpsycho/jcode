...
fn role_catalog_app() -> App {
    ...
    app.remote_available_entries = app.remote_model_options.iter().map(|route| route.model.clone()).collect();
    app
}

fn saved_role_fields(target: AgentModelTarget) -> (Option<String>, Option<ConfigModelRoute>, Option<String>) {
    let cfg = Config::load();
    match target {
...
fn choose_role_entry(app: &mut App, predicate: impl Fn(&crate::tui::PickerEntry) -> bool) {
    ...
    let index = picker.entries.iter().position(predicate).expect("requested role option");
    picker.selected = picker.filtered.iter().position(|candidate| *candidate == index).unwrap();
    picker.column = picker.max_navigable_column();
    app.handle_inline_interactive_key(KeyCode::Enter, KeyModifiers::NONE).unwrap();
}

fn assert_role_target_and_filter(app: &App, target: AgentModelTarget, filter: &str) {
    let picker = app.inline_interactive_state.as_ref().expect("role picker remains open");
...
fn stage_role_catalog_result(app: &mut App) -> std::sync::mpsc::Sender<anyhow::Result<super::super::ModelPickerRoutesResult>> {
    let signature = app.model_picker_cache.as_ref().expect("raw catalog cache").signature.clone();
...
fn agent_role_picker_lists_all_seven_roles() {
    with_temp_jcode_home(|| {
        ...
        let targets: Vec<_> = picker.entries.iter().filter_map(|entry| match entry.action {
            PickerAction::AgentTarget(target) => Some(target),
            _ => None,
        }).collect();
        assert_eq!(targets, vec![
            AgentModelTarget::Main, AgentModelTarget::Swarm, AgentModelTarget::Advisor,
            AgentModelTarget::Review, AgentModelTarget::Judge, AgentModelTarget::Memory,
...
fn agent_role_picker_enter_persists_exact_connection_and_effort_without_switching_main() {
    with_temp_jcode_home(|| {
        ...
        for (target, model, method, provider, legacy) in [
            (AgentModelTarget::Swarm, "gpt-5.5", "jcode-subscription", "Jcode", "gpt-5.5"),
            (AgentModelTarget::Review, "openai/gpt-5.5", "openrouter", "Azure", "openai/gpt-5.5@Azure"),
            (AgentModelTarget::Judge, "gpt-5.5", "openai-compatible:team-work", "Team Work", "team-work:gpt-5.5"),
        ] {
            let mut app = role_catalog_app();
            app.open_agent_model_picker(target);
            choose_role_entry(&mut app, |entry| entry.effort.as_deref() == Some("high")
                && entry.options.iter().any(|option| option.api_method == method && option.provider == provider));
            let (saved_model, saved_route, saved_effort) = saved_role_fields(target);
            assert_eq!(saved_model.as_deref(), Some(legacy));
            assert_eq!(saved_route, Some(ConfigModelRoute {
                model: model.into(), api_method: method.into(), provider_label: provider.into(),
            }));
            ...
            assert_eq!(Config::load().provider.default_model.as_deref(), Some("saved-main"));
        }
    });
}
...
fn agent_role_picker_memory_allows_catalog_models_outside_claude_and_openai() {
    with_temp_jcode_home(|| {
        ...
        choose_role_entry(&mut app, |entry| entry.name == "gemini-2.5-pro"
            && entry.options.iter().any(|option| option.api_method == "code-assist-oauth" && option.available));
        let (_, route, effort) = saved_role_fields(AgentModelTarget::Memory);
        assert_eq!(route, Some(ConfigModelRoute {
            model: "gemini-2.5-pro".into(), api_method: "code-assist-oauth".into(), provider_label: "Gemini".into(),
        }));
...
fn agent_role_picker_inherit_clears_model_route_and_effort_for_each_saved_role() {
    with_temp_jcode_home(|| {
        for (target, role) in [
            ...
            Config::set_agent_model_selection(role, Some(&ConfigModelRoute {
                model: "gpt-5.5".into(), api_method: "jcode-subscription".into(), provider_label: "Jcode".into(),
            }), Some("gpt-5.5"), Some("high")).unwrap();
            let mut app = role_catalog_app();
            app.open_agent_model_picker(target);
            choose_role_entry(&mut app, |entry| matches!(entry.action,
                PickerAction::AgentModelChoice { clear_override: true, .. }));
...
fn agent_role_picker_unavailable_saved_routes_and_loading_placeholders_cannot_be_saved() {
    with_temp_jcode_home(|| {
        let saved = ConfigModelRoute {
            model: "missing-worker".into(), api_method: "jcode-subscription".into(), provider_label: "Jcode".into(),
        };
        Config::set_agent_model_selection(AgentModelRole::Swarm, Some(&saved), None, Some("high")).unwrap();
        let mut app = role_catalog_app();
        app.open_agent_model_picker(AgentModelTarget::Swarm);
        let missing = app.inline_interactive_state.as_ref().unwrap().entries.iter()
            .find(|entry| entry.name.starts_with("missing-worker")).unwrap();
        ...
        for method in ["current", "remote-catalog"] {
            ...
            let picker = app.inline_interactive_state.as_ref().unwrap();
            let placeholder = picker.entries.iter().find(|entry| entry.options.iter().any(|option| option.api_method == method)).unwrap();
            assert!(placeholder.options.iter().all(|option| !option.available));
            choose_role_entry(&mut app, |entry| entry.options.iter().any(|option| option.api_method == method));
            assert_eq!(saved_role_fields(AgentModelTarget::Ambient), (None, None, None));
        }
    });
}
...
fn agent_role_picker_cache_refresh_and_async_completion_preserve_role_and_filter() {
    with_temp_jcode_home(|| {
        ...
        app.remote_model_options.push(role_route("gpt-5.6", "openai-oauth", "OpenAI"));
        app.invalidate_model_picker_cache();
        app.refresh_open_model_picker_after_catalog_update();
        assert_role_target_and_filter(&app, AgentModelTarget::Judge, "gpt");
        // Finally deliver the same kind of channel result used by the worker.
        let sender = stage_role_catalog_result(&mut app);
        assert!(sender.send(Ok(super::super::ModelPickerRoutesResult {
            routes: app.remote_model_options.clone(), routes_ms: 0,
        })).is_ok());
        assert!(app.poll_model_picker_load());
        assert_role_target_and_filter(&app, AgentModelTarget::Judge, "gpt");
        choose_role_entry(&mut app, |entry| entry.effort.as_deref() == Some("high")
            && entry.options.iter().any(|option| option.api_method == "jcode-subscription"));
...
fn agent_role_picker_late_async_completion_does_not_reopen_after_escape() {
    with_temp_jcode_home(|| {
        ...
        let sender = stage_role_catalog_result(&mut app);
        app.handle_inline_interactive_key(KeyCode::Esc, KeyModifiers::NONE).unwrap();
        ...
        assert!(!app.poll_model_picker_load());
        assert!(app.inline_interactive_state.is_none());
        assert_eq!(saved_role_fields(AgentModelTarget::Review), (None, None, None));
    });
}
