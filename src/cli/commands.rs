...
#[path = "commands/model_catalog.rs"]
mod model_catalog;
pub use model_catalog::run_model_command;
#[cfg(test)]
use model_catalog::{
    cli_api_method_display, cli_route_provider_display, collect_cli_model_names,
    filter_cli_model_routes_for_choice,
};
...
fn run_cloud_sessions_dashboard(request: CloudSessionsDashboardRequest) -> Result<()> {
    ...
    if request.open {
        open::that(&output_path).map_err(|error| {
            anyhow::anyhow!("Dashboard was saved but could not be opened: {error}")
        })?;
    }
    Ok(())
}
