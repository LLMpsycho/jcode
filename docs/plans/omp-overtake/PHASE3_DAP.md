...
All focused DAP checks pass with 37 tests. The repository-wide code-size budget still reports unrelated pre-existing drift outside these crates. The largest DAP production file is 527 lines and the largest DAP test file is 600 lines, so this slice remains below the repository file budgets.
